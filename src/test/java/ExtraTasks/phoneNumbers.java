...
public class phoneNumbers {
    ...
    public static void main(String[] args) {
        String phoneNum = "00-44 48 5555 8361"; // "022-198-53-24"
        System.out.println("Expected: 004-448-555-583-61");
        System.out.println("Actual: " + fixedNum(phoneNum));
    }

    private static String fixedNum(String phoneNum) {

        phoneNum = phoneNum.replace(" ","").replace("-","");
        String result = "";
        if(phoneNum.length() % 3 == 2 || phoneNum.length() % 3 == 0) {
            for(int i=0, j=0; i<phoneNum.length(); i++) {
            ...
            for(int i=0, j=0; i<phoneNum.length()-2; i++) {
                result+=phoneNum.charAt(i);
                j++;
                if(j%3==0) {
                    result += "-";
            ...
            result += "-" + phoneNum.substring(phoneNum.length()-2);
        }
        return result;
    }
}
